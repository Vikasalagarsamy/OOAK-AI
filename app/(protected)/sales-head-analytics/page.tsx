...
import ManualSpeakerCorrectionFixed from '@/components/manual-speaker-correction-fixed'
...
export default function SalesHeadAnalyticsDashboard() {
  ...
  const [manualCorrectionCall, setManualCorrectionCall] = useState<string | null>(null)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-purple-50 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <TabsContent value="transcripts" className="space-y-6">
            ...
            <div className="flex gap-4 items-center">
              ...
              <Button 
                ...
                onClick={async () => {
                  try {
                    const response = await fetch('/api/call-analytics/debug')
                    const data = await response.json()
                    console.log('🔍 Database Status:', data)
                    toast({
                      title: "Database Status",
                      description: `Found ${data.summary?.total_transcriptions || 0} transcriptions and ${data.summary?.total_analytics || 0} analytics. Check console for details.`
                    })
                  } catch (err) {
                            ...
                            <TableCell>
                              <div className="flex space-x-1">
                                ...
                                <Button
                                  size="sm"
                                  variant="outline"
                                  onClick={() => setManualCorrectionCall(call.id)}
                                  title="Manual Speaker Correction - 100% Accuracy"
                                >
        ...
        {manualCorrectionCall && (
          <Dialog open={!!manualCorrectionCall} onOpenChange={() => setManualCorrectionCall(null)}>
            <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>✏️ Manual Speaker Correction - 100% Accuracy</DialogTitle>
              </DialogHeader>
                          <ManualSpeakerCorrectionFixed
              callId={manualCorrectionCall}
              onCorrectionComplete={(result: any) => {
                  console.log('Manual correction completed:', result);
                  setManualCorrectionCall(null);
                  fetchCallAnalyticsData(); // Refresh data
                  toast({
                    title: "Success!",
                    description: `Manual corrections saved with 100% accuracy. AI learned from ${result.corrections_count} segments.`
                  });
                }}
              />
            </DialogContent>
