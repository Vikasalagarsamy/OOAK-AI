...
export default async function BugsPage({
  ...
  const { success, data: bugs } = await getBugs()
  ...
  return (
    <div className="space-y-6">
      ...
      {stats && <BugStatsCards stats={stats as any} />}

      <BugFilters initialFilters={filters} />

      {!success ? (
        <div className="bg-destructive/10 p-4 rounded-md text-destructive">Failed to load bugs</div>
      ) : (
