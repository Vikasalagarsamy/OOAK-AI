...
echo "🔧 Fixing critical API routes with wrong database configuration..."

# Fix clients API
echo "Fixing clients API..."
cat > app/api/clients/route.ts << 'APIEOF'
...
# Fix suppliers API
echo "Fixing suppliers API..."
cat > app/api/suppliers/route.ts << 'APIEOF'
import { NextRequest, NextResponse } from 'next/server'
import { pool } from '@/lib/postgresql-client'
...
echo "✅ Fixed critical API routes!"
