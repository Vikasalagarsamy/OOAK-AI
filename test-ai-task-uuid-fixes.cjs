...
// Original file backed up as: test-ai-task-uuid-fixes.cjs.backup
...
async function testAITasksUUIDHandling() {
  ...
  try {
    ...
    if (createdTask) {
      await supabasequery('DELETE FROM ai_tasks WHERE id = createdTask.id')
      console.log('✅ Test task cleaned up')
    }
    
    if (logEntry) {
      await supabasequery('DELETE FROM task_generation_log WHERE id = logEntry.id')
