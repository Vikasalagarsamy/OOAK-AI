...
DROP FUNCTION IF EXISTS batch_delete_employees;

-- Create a function to delete multiple employees at once with a simpler signature
CREATE OR REPLACE FUNCTION batch_delete_employees(employee_ids UUID[])
RETURNS TABLE(
  deleted_id UUID,
  success BOOLEAN,
  error_message TEXT
  ...
  LOOP
    BEGIN
      ...
      IF EXISTS (
        ...
        PERFORM remove_employee(employee_id);
      ...
      ELSE
        ...
        DELETE FROM employees WHERE id = employee_id RETURNING id INTO result;
