-- Production Task Management Schema Update (FIXED VERSION)
...
DROP POLICY IF EXISTS "Allow all access to task_status_history" ON task_status_history;
DROP POLICY IF EXISTS "Allow all access to task_performance_metrics" ON task_performance_metrics;
...
-- 11. Add constraint to ensure task status integrity
...
-- 12. Add constraint to ensure completion_at is set when status is COMPLETED
...
-- 13. Create function for task archival (soft delete)
...
-- 14. Create comprehensive task analytics view (simplified to avoid conflicts)
...
-- 15. Success status check - FIXED to use proper SQL syntax
SELECT 
  'PRODUCTION TASK MANAGEMENT SCHEMA UPDATE COMPLETED' as update_status,
  'Status change audit trail: ENABLED' as feature_1,
  'Performance metrics tracking: ENABLED' as feature_2,
  'Data integrity constraints: ENABLED' as feature_3,
  'Automatic archival support: ENABLED' as feature_4,
  'Analytics dashboard views: ENABLED' as feature_5,
  NOW() as completed_at; 